  ...
  <body>
    <cfoutput>
      ...
      <cfif NOT structKeyExists(session, "employee")>
        ...
        <cfinvoke component="models.logsheet" method="admin_wfh_history" returnvariable="emp_wfh"/>
    ...
    <section id="contact" class="contact">
      <div class="container my-5">
        <div class="section-title pt-5">
          ...
          <p>Employee WFH Requests</p>
        </div>
      </div>
      <div class="row me-4 mb-2" style="margin-bottom: -2.2rem !important; margin-right: 1.7rem!important;">
      <div class="col-6"></div>
          <div class="col-6 d-flex justify-content-end">
            <a class="btn btn-sm custom-btn mb-2" style="margin-right:0.5rem!important;" href="wfh_application_form.cfm?id=#encrypt(session.employee.id, 'RHMm64ACs6qftCFBz4j+5Q==', 'AES','HEX')#" role="button">Apply WFH</a>
              <a class="btn btn-sm custom-btn me-2 mb-2" href="employee_wfh_history.cfm?id=#encrypt(session.employee.id, 'RHMm64ACs6qftCFBz4j+5Q==', 'AES','HEX')#" role="button"> WFH History</a>
          </div>
    </div>
      <style>
            table{
           ...
           <div class="m-5"> 
           <div class="card p-2">
          <div class="mb-1 mt-3 ms-2 fw-bold" style="font-size: 0.9rem; color:##7d66e3;">
              <label>WFH REQUESTS</label>
          </div>
          <hr>
          <table class="table table-bordered table-striped border-dark"  id="employee_wfh" style=" overflow: hidden;">
            ...
            <tbody style="background-color:##FEF7F5">
              <cfloop query="emp_wfh">
                <tr class="text-center" style="font-size:small; vertical-align: middle; height: 50px; ">
                  <td>#k#</td>
                  <td>#emp_wfh.first_name# #emp_wfh.last_name#</td>
                  <td>#datetimeformat(emp_wfh.start_date, 'yyyy-mmm-dd')# (#emp_wfh.start_period#)</td>
                  <td>#datetimeformat(emp_wfh.end_date, 'yyyy-mmm-dd')# (#emp_wfh.end_period#)</td>
                  <td> 
                    <cfif listlen(emp_wfh.total_days,".") EQ 2>
                      <cfif listGetAt(#emp_wfh.total_days#, 1,".") EQ 0 AND listGetAt(#emp_wfh.total_days#, 2,".") EQ 5>
                        Half day
                      </cfif>
                      <cfif listGetAt(#emp_wfh.total_days#, 1,".") GTE 1 AND listGetAt(#emp_wfh.total_days#, 2,".") EQ 5>
                       #listGetAt(#emp_wfh.total_days#, 1,".")# and Half days
                      </cfif>
                      <cfif emp_wfh.total_days EQ 1>
                       1 Day
                      </cfif>
                    <cfelseif emp_wfh.total_days LTE 1>
                       #emp_wfh.total_days# Day
                    <cfelse>
                      #emp_wfh.total_days# Days
                    </cfif>
                  </td>
                  <td style="text-align: justify;">#emp_wfh.reason#</td>
                  <td><cfif emp_wfh.approved_status EQ 1>Approved<cfelse>Pending</cfif></td>
                </tr>
              <cfset k++>
              </cfloop>
            </tbody>
           ...
           <div class="row">
               ...
               <div class="col-8 d-flex align-items-center" style="height: 40px; padding-left: 250px; font-family: Inter, sans-serif;">
                ...
                <input type="text" class="form-control ms-1 mb-4 mt-3 pe-5" id="record-info" value="Total Record: #queryRecordCount(emp_wfh)#" readonly  
