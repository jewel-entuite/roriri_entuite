<cfcomponent>
	...
	<cffunction name="getCareerLevels" access="remote" returntype="any" output="false">
	    <cfargument name="departmentId" type="numeric" required="true">
	    <cftry>
	        <cfquery name="careerLevels" datasource="yourDatasource">
	            SELECT id, name FROM career_level 
	            WHERE department_id = <cfqueryparam value="#arguments.departmentId#" cfsqltype="cf_sql_integer">
	        </cfquery>

	        <cfset result = []>
	        <cfloop query="careerLevels">
	            <cfset arrayAppend(result, { "id": careerLevels.id, "name": careerLevels.name })>
	        </cfloop>
    ...
    <cffunction name="getPositions" access="remote" returntype="any" output="false">
	    <cfargument name="careerLevelId" type="numeric" required="true">

	    <cfquery name="positions" datasource="yourDatasource">
	        SELECT id, designation FROM designation 
	        WHERE career_level_id = <cfqueryparam value="#arguments.careerLevelId#" cfsqltype="cf_sql_integer">
	    </cfquery>

	    <cfset result = []>
	    <cfloop query="positions">
	        <cfset arrayAppend(result, { "id": positions.id, "designation": positions.designation })>
	    </cfloop>

	    <cfreturn serializeJSON(result)>
	</cffunction>

	<cffunction name="getCareerLevel">
		<cfquery name="careerLevel">
			SELECT * FROM career_level
		</cfquery>
		<cfreturn careerLevel>
	</cffunction>
